...
const RIESGO_ORDEN: Record<Riesgo, number> = { rojo: 0, amarillo: 1, verde: 2 }
...
export default function PanelINEN() {
  ...
  const enriquecidos = useMemo(
    () =>
      pacientes
        .map((p) => ({ p, ev: evaluarRiesgo(p), accion: accionRecomendadaINEN(p) }))
        .sort((a, b) => {
          const r = RIESGO_ORDEN[a.ev.nivel] - RIESGO_ORDEN[b.ev.nivel]
  ...
  const filtrados = enriquecidos.filter(({ p, ev }) => {
    ...
    if (fRiesgo !== 'todos' && ev.nivel !== fRiesgo) return false
    if (fSospecha !== 'todas' && p.tipoSospecha !== fSospecha) return false
    if (busqueda && !`${p.nombres} ${p.apellidos} ${p.procedencia}`.toLowerCase().includes(busqueda.toLowerCase()))
      return false
    return true
  })
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <SectionTitle sub="Seguimiento operativo de la navegación diagnóstica. Prioriza por riesgo de demora.">
          Panel de pacientes
        </SectionTitle>
        <span className="text-sm text-tinta/55">{filtrados.length} de {pacientes.length} pacientes</span>
      </div>

      {/* Resumen por riesgo */}
      <div className="grid grid-cols-3 gap-3">
        {([
          { n: 'rojo', label: 'Riesgo alto', desc: 'Requiere acción hoy' },
          { n: 'amarillo', label: 'Posible demora', desc: 'Revisar pronto' },
          { n: 'verde', label: 'Avance normal', desc: 'Seguimiento estándar' },
        ] as { n: Riesgo; label: string; desc: string }[]).map((c) => (
          <button
            key={c.n}
            onClick={() => setFRiesgo(fRiesgo === c.n ? 'todos' : c.n)}
            className={`tarjeta ac-surface p-4 text-left transition ${fRiesgo === c.n ? 'ring-2 ring-marca-400' : ''}`}
          >
            <div className="flex items-center justify-between">
              <SemaforoRiesgo nivel={c.n} size="sm" />
              <span className="font-display text-2xl font-extrabold text-tinta ac-ink">{conteo[c.n]}</span>
            </div>
      ...
      <Card>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select label="Etapa" value={fEtapa} onChange={setFEtapa} options={['todas', ...ETAPAS_ORDEN]} />
          <Select label="Sospecha" value={fSospecha} onChange={setFSospecha} options={['todas', 'Mama', 'Cérvix']} />
        </div>
      </Card>
            ...
            {filtrados.map(({ p, ev, accion }) => (
              ...
              >
                <td className="px-4 py-3">
                  <p className="font-semibold text-tinta ac-ink">{p.nombres} {p.apellidos}</p>
                  <p className="text-xs text-tinta/50 ac-muted">{p.edad} años · {p.procedencia}{p.esProvincia ? ' (provincia)' : ''}</p>
                </td>
                ...
                <td className="px-4 py-3">
                  <span className={`font-bold ${p.diasSinAvance > 7 ? 'text-riesgo' : p.diasSinAvance >= 3 ? 'text-[#9a7400]' : 'text-tinta/60'}`}>
                    {p.diasSinAvance} días
                  </span>
                </td>
                <td className="px-4 py-3"><SemaforoRiesgo nivel={ev.nivel} size="sm" /></td>
                <td className="px-4 py-3">
        ...
        {filtrados.map(({ p, ev, accion }) => (
          <Card key={p.id} onClick={() => seleccionarPaciente(p.id)}>
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-semibold text-tinta ac-ink">{p.nombres} {p.apellidos}</p>
                <p className="text-xs text-tinta/50 ac-muted">{p.edad} años · {p.procedencia} · {p.tipoSospecha}</p>
              </div>
              <SemaforoRiesgo nivel={ev.nivel} size="sm" />
            </div>
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: string[] }) {
  return (
    <label className="inline-flex items-center gap-2 bg-white border border-black/10 rounded-2xl px-3 py-2 text-sm">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)} className="outline-none bg-transparent font-medium text-tinta ac-ink">
        {options.map((o) => (
          <option key={o} value={o}>{o === 'todas' || o === 'todos' ? 'Todas' : o}</option>
        ))}
      </select>
    </label>
  )
